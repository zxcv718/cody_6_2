...
4. **차이가 보이려면 diff가 충분히 커야 합니다.** 위는 2,343줄 diff의 결과입니다.
   새로 clone한 저장소에서 18줄만 고치고 같은 실험을 했더니 `0.0`과 `1.0`이
   56 대 59토큰, 불릿 수도 동일했습니다 — 요약할 방법 자체가 몇 가지 없기 때문입니다.
   재현성 확인은 항상 되지만, **변주 폭은 입력의 풍부함에 비례합니다.**
